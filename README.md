...
```
venv\Scripts\python scripts\tests\test_pure.py     # 골든 테스트 242건
```
...
**설정 파일 하나가 없어지면 책이 망가진다 — 그리고 아무도 모른다.** 어떤 책의
...
**한 영역의 잘못된 꼬리표가 페이지 전체를 무너뜨린다.** 레이아웃 모델이 오른쪽 단의
...
| 항목 | 값 |
|---|---|
| 처리량 | 8권 6,577쪽 (스캔본 쪽당 6.8초, 내장 텍스트층 쪽당 3.8초) |
| 페이지 실패 | 0 |
| 그림 추출 | 7,993개 (깨진 링크 0, 고아 파일 0) |
| 본문 유사도 (세대 간) | Dice 99.31~99.98% |
| **원본 쪽 대조** | **문자 일치율 93.3% · 낱말 회수율 94.3%** — 8권 16쪽 전사 대조 |
| 기계 교차 대조 | 산문 회수 95.6% · 정밀도 98.5% — 내장층 있는 책 504쪽 |
| 인쇄 쪽번호 회수 | 90.5~97.2% · 8권 모두 중복 0 · 역행 0 |
| 골든 테스트 | 242건 (모델·외부 프로세스 없이 실행) |
...
| `scripts/tests/test_pure.py` | 순수 함수 골든 테스트 242건 |
